...
def generate_watershed_section(
    ...
    if include_shape_indicators and any(
        [form_factor, compactness, circularity, elongation]
    ):
        lines.extend(
            [
                "",
        ...
        if all([form_factor, compactness, circularity, elongation]):
            ...
            if form_factor is not None:
                indicator_data.append(
                    ("Wskaźnik formy Hortona (Cf)", f"{form_factor:.3f}", "-")
                )
            if compactness is not None:
                indicator_data.append(
                    ("Wsp. zwartości Graveliusa (Cz)", f"{compactness:.3f}", "-")
                )
            if circularity is not None:
                indicator_data.append(
                    ("Wsp. kolistości Millera (Ck)", f"{circularity:.3f}", "-")
                )
            if elongation is not None:
                indicator_data.append(
                    ("Wsp. wydłużenia Schumma (Ce)", f"{elongation:.3f}", "-")
                )
